...
def index():
    if request.method == "GET":
        ...
        if (
            (problem_num > NUM_OF_PROBLEMS)
            or (problem_num < 1)
            or not problem_num.isdigit()
        ):
            flash("Invalid problem number")
            return redirect("/")
...
def results():
    ...
    update_spreadsheet(
        datetime.datetime.now(), email, name, problem_num, code_content, output
    )

    return render_template(
        "results.html", errors=errors, output=output, problem_num=problem_num
    )
...
def quicksort_leaderboard(arr):
    ...
    return (
        quicksort_leaderboard(items_greater)
        + [pivot]
        + quicksort_leaderboard(items_lesser)
    )
...
def leaderboard():
    ...
    for i in range(5, 10):
        leaderboard.append(
            [
                leaderboard_sheet.row_values(i)[2],
                int(leaderboard_sheet.row_values(i)[12]),
            ]
        )
